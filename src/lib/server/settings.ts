...
export interface ForgeSettings {
	enabled: boolean;
	/**
	 * Steps the driver may start per scheduler tick, across every epic.
	 *
	 * This is the wall-clock dial. The tick is five minutes, so 1 is at most
	 * twelve steps an hour whatever the models do. 0 pauses everything — and
	 * because it is a settings row rather than a flag in a process, the pause
	 * survives a restart, which is the only kind of pause worth having for
	 * something that runs for days.
	 */
	stepsPerTick: number;
	...
	/** Attempts a task or a sprint gets before it parks. */
	maxAttempts: number;
	/**
	 * Spend ceilings, in USD. 0 is no ceiling.
	 *
	 * Separate from stepsPerTick because speed is two independent things, and one
	 * number cannot say the thing people actually want: go as fast as you like,
	 * and never spend more than this.
	 */
	maxUsdPerEpic: number;
	maxUsdPerDay: number;
	/**
	 * Skip the charter approval and run from brief to pull request unattended.
	 *
	 * Off by default, so the one moment a person is asked for anything is the one
	 * where the checks are frozen. Turning it on is a real choice and should look
	 * like one.
	 */
 ...
 * spend.
 */
export function normaliseForgeSettings(raw: Record<string, unknown>): ForgeSettings {
	...
	// Money is not counted in whole units — a £12.50 ceiling has to survive the
	// round trip, and flooring it is how a cap quietly becomes a different cap.
	const usd = (v: unknown, fallback: number) => {
		const parsed = Number(v);
		return Number.isFinite(parsed) ? Math.min(10_000, Math.max(0, parsed)) : fallback;
	};
	return {
		enabled: raw.enabled === true,
		stepsPerTick: n(raw.stepsPerTick, DEFAULT_FORGE.stepsPerTick, 0, 20),
		concurrentEpics: n(raw.concurrentEpics, DEFAULT_FORGE.concurrentEpics, 1, 10),
		concurrentTasks: n(raw.concurrentTasks, DEFAULT_FORGE.concurrentTasks, 1, 10),
		maxStepsPerTask: n(raw.maxStepsPerTask, DEFAULT_FORGE.maxStepsPerTask, 5, 200),
		maxAttempts: n(raw.maxAttempts, DEFAULT_FORGE.maxAttempts, 1, 10),
		maxUsdPerEpic: usd(raw.maxUsdPerEpic, DEFAULT_FORGE.maxUsdPerEpic),
		maxUsdPerDay: usd(raw.maxUsdPerDay, DEFAULT_FORGE.maxUsdPerDay),
		autoApproveCharter: raw.autoApproveCharter === true
	};
}
