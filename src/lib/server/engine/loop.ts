		...
		try {
			...
			for await (const ev of stream) {
				if (ev.type === 'text') {
					...
					// Dropped on the floor before this existed. A reasoning model
					// can spend half a minute here before its first token of answer
					// or its first tool call, and chat showed nothing for all of it
					// — the spinner alone, which looks exactly like a hung run. It
					// was reported as the model being slow; the model was working
					// and the interface would not say so.
