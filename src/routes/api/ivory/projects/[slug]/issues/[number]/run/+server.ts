...
import { IvoryRunError, startReadRun } from '$lib/server/ivory/run';
...
// Run the reader on one of a project's issues. The issue stays open; the owner closes it.
export const POST: RequestHandler = async ({ locals, params }) => {
	const user = requireCoder(locals);
	const number = Number(params.number);
	if (!Number.isInteger(number) || number < 1) error(400, 'Not an issue number');
	try {
		const { chatId, job } = await startReadRun({ slug: params.slug, issueNumber: number, userId: user.id });
