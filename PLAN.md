...
6. **Deep research** — a pipeline, not a single call: plan → rounds of parallel searches → fetch + read pages → **consolidate what was read into a carried brief of findings and open gaps** → search those gaps → synthesis with citations. Each round narrows on the last one's gaps rather than repeating its breadth, and the brief is what keeps the context flat across rounds. Admin sets engine, max tokens, timeout and the round ceiling; a per-message **effort** control beside the composer's toggle (Quick / Balanced / Exhaustive) spends a fraction of that ceiling. Progress UI streams the stages. Output can be saved to the Library in one click.
