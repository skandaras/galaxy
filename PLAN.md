...
9. **System prompts** — per-task, editable in admin, **versioned with history and rollback**. These sit above repo-level `AGENTS.md`/`CLAUDE.md`-style files, which the coding agent still reads inside each repo. Underneath them sits a **house style** the prose-writing tasks share: `OUTPUT_FORMAT` governs the shape of a reply and `HOUSE_VOICE` its sentences, both composed at call time by `systemPromptFor` rather than seeded, so a reworded rule reaches every install without a prompt migration and without treading on an edit somebody made their own. The owner adds to it in one place (Admin → Settings → House style) instead of in fourteen textareas, and their half wins where the two disagree. The tasks whose reply is a fixed shape — chat titles, run summaries, the memory audit, the alignment assessor — are deliberately outside it.
