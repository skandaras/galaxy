...
5. **Web search** — pluggable provider interface (Brave / Tavily / SearXNG); admin sets provider, result count, timeouts. Runs as a tool inside the engine loop, **one search per model turn**: a second query in the same turn is refused without spending the allowance, so the model reads what came back before it writes the next query. The turn allowance was never the problem — six searches over six turns is research, six in one message is six guesses.

6. **Deep research** — a pipeline, not a single call: **one orienting search** → fetch + read pages → **consolidate what was read into a carried brief of findings and open gaps** → search those gaps, and **follow the links those pages cited** → repeat → synthesis with citations. A round can be handed addresses as well as queries: one the person put in their own message, or one a source pointed at — a page citing where it got its numbers is a better lead than any query, and following it is the one thing a fixed pipeline could not previously do. Planning is shown what the Library and the Cortex lattice already hold on the subject, for aiming the opening query; those are never sources and cannot be cited. The first round is deliberately a single query, because it is the only round planned before anything has come back: its job is to reveal how the subject is actually covered so the next round can be aimed. Later rounds stay narrow and each narrows on the last one's gaps; the brief is what keeps the context flat across rounds. **Effort buys rounds and reading, not breadth** — Quick / Balanced / Exhaustive spend a fraction of the admin's round and page ceilings while queries per round stays where the admin set it. Admin sets engine, max tokens, timeout, the round ceiling and how wide a round may go. Progress UI streams the stages. Output can be saved to the Library in one click.
...
9. **System prompts** — the default for each task lives in code (`engine/prompts.ts`); a `task_configs` row holds an override only where the owner wrote one, **versioned with history and rollback**. An unedited task therefore tracks the code, so a reworded default reaches every install with no migration, and an edited one is never touched. These sit above repo-level `AGENTS.md`/`CLAUDE.md`-style files, which the coding agent still reads inside each repo. Underneath them sits a **house style** the prose-writing tasks share, composed at call time by `systemPromptFor`: `HOUSE_VOICE` governs how a reply sounds and reaches all of `PROSE_TASKS`, while `OUTPUT_FORMAT` governs its shape and reaches only `LAYOUT_TASKS` — the ones that answer in prose rather than with a JSON object. The owner adds to it in one place (Admin → Settings → House style) instead of in eighteen textareas, and their half wins where the two disagree. The tasks whose reply is a fixed shape — chat titles, run summaries, the memory audit, the alignment assessor — are outside both.

10. **Library** — flat `/data/library/*.md` per spec; a shelf of folders with documents nested inside documents beneath them, drag to re-file; documents open formatted and are edited as a second step; uploads converted to markdown; a cached **snippet** per doc (first lines or a one-time model summary) so the on-load digest stays cheap; FTS5 full-text search; docs readable by all agents; agent-authored docs flagged as such.
...
14. **Theme / UI** — futuristic-minimalist space aesthetic: pre-rendered ASCII-art galaxy as the ambient background (subtle, low-contrast; optional slow canvas shimmer). All theming = CSS custom properties driven from theme JSON: colours, fonts, backgrounds, highlights, button styling, layout density — editable in Theme settings with live preview, per-user selection, exportable presets. **Fonts are picked from dropdowns, never typed**: the theme stores an id from a catalogue (`src/lib/fonts.ts`) whose every stack ends in a generic family, so a stack without a fallback is unrepresentable. Interface and code fonts are separate (Quicksand and Source Code Pro, both bundled as woff2 so the defaults render everywhere); numbers take the code font so figures in a column line up; and the ASCII backdrop sits outside the theme system on a fixed stack, because its shape depends on every glyph being the same width. Contrast is a first-class constraint — see `docs/ACCESSIBILITY.md`.
...
17. **Ivory Tower** (`docs/ivory-tower/`) — theory-only research: literature synthesis, and mapping a mechanism from one field onto another. **The Shelf** is a separate GitHub repository (Admin → Settings → Shelf) holding project briefs, one note per source and claim files; `projects/` is flat and a project sits under every discipline its brief names. Its **board is that repository's Issues**, by label (`project:`, `discipline:`, `agent:`), and Galaxy stores no project or task state: pages read through a one-minute cache, and status lives on GitHub. Galaxy's own Boards are unrelated. The view at `/ivory` is behind the coding grant, because it uses the shared GitHub token. Admin's "Set up Shelf" writes the templates from `docs/ivory-tower/seed/` and creates the labels, and never overwrites. **Agents** run only when a person starts them: `ivory-read` reads the sources an issue names into one note per source, with the `research` toolset (`paper_search` on OpenAlex or Semantic Scholar, `read_paper` for full texts from Europe PMC, arXiv, CORE, Semantic Scholar and open repository copies, `fetch_url`, `shelf_read`, `shelf_write`) and Shelf writes confined to that project's `notes/` in code. A note is written only if each claim's quote appears in something the run read, and the runner, not the agent, comments on the issue. `ivory-plan` proposes a project's next tasks, kill-criteria checks first, with read-only tools and `propose_tasks`; the issues are created by the approval handler once the owner has edited and approved the plan, never by the agent. `ivory-synthesise` turns notes into mapping claims and `ivory-redteam` attacks them; a claim is never reviewed by its author's model family, checked in code at every write. Re-planning is given the closed tasks, the notes and the claims, so each round builds on the last. Projects are created from a form at `/ivory/new`, and each carries a one- or two-line **status** in a marked section of its project issue, set by the latest run through `set_status` and written by the runner.
